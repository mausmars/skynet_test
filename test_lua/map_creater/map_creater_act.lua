...
local EventType = {
    ...
    ELITE = 102, -- 102-精英战斗(冰冻npc2)       对应 T_Stage 表
    BOX = 2, -- 2-冰冻宝箱                      对应 T_Item 表
    NPC1 = 3, -- 3-对话(冰冻npc1)               对应 T_Stage 表
    STONE = 4, -- 4-冰冻巨石
}
...
function MapCreater:findPaths(endCount, totalStep, fight_random_range, stone_random_range, expect_counts, endpoint_eventtype)
    --平均步数
    ...
    -- 设置事件类型
    self:set_event_type(findContext, fight_random_range, stone_random_range, expect_counts, endpoint_eventtype)
...
-- 设置事件类型
function MapCreater:set_event_type(findContext, fight_random_range, stone_random_range, expectcounts, endpoint_eventtype)
    --查找路徑最長的通路
    ...
    for _, pc in pairs(findContext.pathContexts) do
        ...
        local stone_step = math.random(stone_random_range[1], stone_random_range[2])
        local index = 0
        ...
        while (true) do
            ...
            if counts[ExpectCountType.FIGHT] < expectcounts[ExpectCountType.FIGHT] then
                if fight_interval >= fight_step + 1 then
                    ...
                    if mp.event_type == EventType.EMPTY and not self:around_point_contain_fight(mp.x, mp.y, findContext.usedPoints, fight_random_range[1]) then
                        ...
                        fight_interval = 0
                        counts[ExpectCountType.FIGHT] = counts[ExpectCountType.FIGHT] + 1
                    end
                end
            end

            if index == pc.pathSize - (stone_step) - 2 then
                if counts[ExpectCountType.STONE] < expectcounts[ExpectCountType.STONE] then
                    local mp = findContext.usedPoints[pointKey]
                    mp.event_type = EventType.STONE
                    counts[ExpectCountType.STONE] = counts[ExpectCountType.STONE] + 1
                end
            end
        end
        -- 末点
        ...
        elseif counts[ExpectCountType.BOX] < expectcounts[ExpectCountType.BOX] then
            end_point.event_type = EventType.BOX
            counts[ExpectCountType.BOX] = counts[ExpectCountType.BOX] + 1
        elseif counts[ExpectCountType.NPC1] < expectcounts[ExpectCountType.NPC1] then
            end_point.event_type = EventType.NPC1
            counts[ExpectCountType.NPC1] = counts[ExpectCountType.NPC1] + 1
...
function MapCreater:printMap(findContext)
    ...
    print("[#]EMPTY [*]START [@]FIGHT [B]BOSS [E]ELITE [N]NPC1 [S]STONE [X]BOX")
    if findContext.isSuccess then
        print("Success!")
    else
        print("Fail!")
...
local function test()
    ...
    for i = 1, totalCount do
        --print("--------------------------------------------------------------------------------------")
        local findContext = mapCreater:findPaths(endCount, totalStep, fight_random_range, stone_random_range, expect_counts, endpoint_eventtype)
